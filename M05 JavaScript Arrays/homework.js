...
function arrayContiene(array, elemento) {
  ...
  const existeElemento = array.includes(elemento);
...
function empiezaConNueve(num) {
  ...
  const primerNumeroDelArray = array[0];

  if (primerNumeroDelArray === "9") {
    return true;
  } else {
    return false;
  }
...
function continueStatement(num) {
  ...
  for (let i = 1; i < 10; i++) {
    ...
    nuevoArreglo.push(acum);
  }

  return nuevoArreglo;
}

const result = continueStatement(-5);
console.log(result);
